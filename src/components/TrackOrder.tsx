...
export function TrackOrder({ tableId, tenantId, refreshTrigger }: TrackOrderProps) {
      ...
      .on(
        ...
        (payload) => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            // Client-side filter: only process orders for this table
            if (newOrder.table_id !== tableId) return;
            
            toast.success("🎉 Order Placed!", {
              description: "We've received your order and will update you shortly!",
              duration: 4000,
            });
            
            setOrders(prevOrders => [...prevOrders, newOrder]);
            previousStatusRef.current.set(newOrder.order_id, newOrder.status);
            return;
          }
          ...
          if (payload.eventType === 'UPDATE') {
            ...
            // Client-side filter: only process orders for this table
            if (updatedOrder.table_id !== tableId) return;
            const newStatus = updatedOrder.status;
            const newPaymentStatus = updatedOrder.payment_status;
            const orderId = updatedOrder.order_id;
            
            // Check if order should be visible (10-minute rule for completed/rejected)
            const tenMinutesAgo = new Date(Date.now() - 10 * 60 * 1000).toISOString();
            const shouldShow = !(
              (newStatus === 'completed' || newStatus === 'rejected') && 
              updatedOrder.last_updated_at <= tenMinutesAgo
            );
            ...
            setOrders(prevOrders => {
                ...
                if (newPaymentStatus === 'paid') {
                  toast.success("💰 Payment Confirmed!", {
                    description: updatedOrder.payment_mode 
                      ? `We've received your ${updatedOrder.payment_mode.toUpperCase()} payment. Thank you!`
                ...
                switch (newStatus) {
                  case 'accepted':
                    toast.success("✅ Great News!", {
                      description: "Your order has been accepted! We're getting started on it now.",
                      duration: 4000,
                    });
                    break;
                  case 'cooking':
                    toast.info("👨‍🍳 Cooking in Progress!", {
                      description: "Our chef is preparing your delicious meal. It won't be long now!",
                      duration: 4000,
                    });
                    break;
                  case 'completed':
                    toast.success("🍽️ Ready to Enjoy!", {
                      description: "Your order is complete! We hope you love it. Bon appétit!",
                      duration: 5000,
                    });
                  ...
                  case 'rejected':
                    toast.error("❌ Order Could Not Be Processed", {
                      description: "We're sorry, but we couldn't complete your order. A refund will be processed shortly.",
                      duration: 5000,
                    });
                    break;
                }
              ...
              // If order should not be shown, remove it
              if (!shouldShow) {
  ...
  const fetchOrder = async () => {
    ...
    try {
      ...
      // Filter out orders completed/rejected for more than 10 minutes
      const tenMinutesAgo = new Date(Date.now() - 10 * 60 * 1000).toISOString();
      const filteredOrders = fetchedOrders.filter((order: Order) => {
        if (order.status === 'completed' || order.status === 'rejected') {
          return order.last_updated_at > tenMinutesAgo;
        }
        return true;
      });
