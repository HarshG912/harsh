...
export default function TenantCart() {
  ...
  const isRazorpayEnabled = (settings as any)?.razorpay_enabled || false;
  ...
  const handleConfirmOrder = async () => {
    ...
    // If Razorpay is enabled, use Razorpay flow
    if (isRazorpayEnabled && razorpayLoaded) {
  ...
  const handleRazorpayPayment = async () => {
    ...
    try {
      ...
      const options = {
        ...
        name: tenant?.restaurant_name || 'Restaurant',
        description: `Order for Table ${tableNumber}`,
        order_id: razorpayData.razorpay_order_id,
        handler: async (response: any) => {
          await handlePaymentSuccess(response, subtotal, serviceChargeAmount, grandTotal);
        },
        prefill: {
