...
export default function Cook() {
  ...
  useEffect(() => {
    const channel = supabase.channel('cook-orders').on('postgres_changes', {
      event: 'INSERT',
      schema: 'public',
      table: 'orders'
    }, payload => {
  ...
  const checkAuth = async () => {
    ...
    const {
      data: roles
    } = await supabase.from("user_roles").select("role, tenant_id").eq("user_id", session.user.id);
    ...
    const hasPermission = hasAnyPermission(userRoles, ['chef']);
    ...
    const isManagerRole = userRoles.some(r => r === 'manager' || r === 'tenant_admin');
  ...
  const {
    ...
    queryFn: async () => {
      // Filter orders: show all active orders + completed/rejected orders from last 10 minutes
      const tenMinutesAgo = new Date(Date.now() - 10 * 60 * 1000).toISOString();
      const {
        data,
        error
      } = await supabase.from("orders").select("*").eq("tenant_id", tenantId).or(`status.in.(pending,accepted,cooking,ready),and(status.in.(completed,rejected),last_updated_at.gte.${tenMinutesAgo})`).order("created_at", {
        ascending: false
      });
  ...
  const handleDownloadBill = async (order: Order) => {
    try {
      ...
      const {
        data: settings,
        error
      } = await supabase.from('tenant_settings').select('restaurant_name, restaurant_address, merchant_upi_id, service_charge').eq('tenant_id', tenantId).single();
      if (error) throw error;
      const billData = orderToBillData(order, settings.restaurant_name || 'Restaurant', settings.restaurant_address || '', settings.merchant_upi_id || '', settings.service_charge || 0);
      ...
      <div className="pt-14 sm:pt-16 md:pt-20">
        ...
        <Tabs value={filter} onValueChange={setFilter} className="w-full">
          ...
          {isLoading ? <div className="text-center py-12">
              ...
              return <Card key={order.order_id} className="rounded-xl shadow-lg hover:shadow-xl transition-shadow">
                    <CardHeader>
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex flex-col gap-2 items-end">
                          ...
                          {order.payment_status === "paid" && order.payment_mode ? <Badge variant="default">
                              {order.payment_mode.toUpperCase()}
                            </Badge> : order.payment_status === "paid" ? <Badge variant="default">
                              PAID
                            </Badge> : <Badge variant="destructive">
                              UNPAID
                            </Badge>}
                          {order.payment_claimed && order.payment_status !== "paid" && <Badge variant="outline" className="bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200 border-yellow-300 dark:border-yellow-700">
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="space-y-2">
                        {items.map((item, index) => <div key={index} className="flex justify-between text-sm">
                            <span>
                      ...
                      <div className="space-y-2 pt-2">
                        <Button onClick={() => togglePaymentStatus.mutate({
                      orderId: order.id!,
                      currentStatus: order.payment_status
                    })} disabled={togglePaymentStatus.isPending} variant={order.payment_status === "paid" ? "outline" : "default"} className="w-full">
                          {order.payment_status === "paid" ? "Mark as Unpaid" : "Mark as Paid"}
                        </Button>
                        {order.payment_status === "paid" && <Button onClick={() => handleDownloadBill(order)} variant="outline" className="w-full">
                            <Download className="mr-2 h-4 w-4" />
                            Download Bill
                          </Button>}
                        {order.status === "pending" && order.payment_claimed && <Button onClick={() => updateOrderStatus.mutate({
                      orderId: order.id!,
                      newStatus: "accepted",
                      markPaid: true,
                      cookName: cookName
                    })} disabled={updateOrderStatus.isPending} className="w-full h-11 bg-lime-500 hover:bg-lime-400">
                            Accept & Mark as Paid
                          </Button>}
                        {order.status !== "completed" && order.status !== "rejected" && order.status !== "pending" && <Button onClick={() => updateOrderStatus.mutate({
                      orderId: order.id!,
                      newStatus: getNextStatus(order.status),
                      cookName: cookName
                    })} disabled={updateOrderStatus.isPending} className="w-full h-11">
                      ...
                      cookName: cookName
                        ...
                        {(order.status === "pending" || order.status === "accepted") && <Button onClick={() => updateOrderStatus.mutate({
                      orderId: order.id!,
                      newStatus: "rejected"
                    })} disabled={updateOrderStatus.isPending} variant="destructive" className="w-full">
                            Reject Order
                          </Button>}
                      </div>
                    </CardContent>
